  ...
  return (
    <div className="container mx-auto p-4 max-w-3xl">
      <Link href="/polls" className="mb-4 inline-block">
        <Button variant="outline" className="border-glass-border bg-background/50 hover:bg-accent/30 transition-all duration-300">
          ...
          <div className="space-y-4">
            {poll.options.map((option, index) => {
              ...
              return (
                <div 
                  key={option.id} 
                  className={`border border-glass-border bg-background/50 backdrop-blur-sm rounded-lg p-4 transition-all duration-300 ${selectedOption === option.id && !hasVoted ? 'glow border-primary/50' : ''} ${hasVoted ? 'hover:border-glass-border' : 'hover:border-primary/30 cursor-pointer'}`}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center">
                      ...
                      <label htmlFor={option.id} className="text-md font-medium">
                        <span className="text-xs text-primary/70 mr-2">{index + 1}.</span>
                        {option.text}
                      </label>
                    </div>
                    {hasVoted && (
